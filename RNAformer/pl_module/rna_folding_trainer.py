...
class RNAFoldingTrainer(pl.LightningModule):
    ...
    def __init__(
        ...
        if not hasattr(cfg_model, "binary_output") or cfg_model.binary_output:
            ...
            # self.loss_train = torch.nn.NLLLoss(ignore_index=self.ignore_index, reduction='none',)
            # self.loss_valid = torch.nn.NLLLoss(ignore_index=self.ignore_index, reduction='none',)

            # self.loss_train = torch.nn.BCELoss(reduction='none',)
            # self.loss_valid = torch.nn.BCELoss(reduction='none',)
            assert(True==False)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # logging.info("self.loss.shape 0: {}".format(loss.shape))
        ...
        # self.log("pred.max", pred.min(), on_step=True, on_epoch=False,)
