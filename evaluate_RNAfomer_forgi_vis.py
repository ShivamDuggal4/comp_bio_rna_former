...
def evaluate_RNAformer(model, test_sets, viz_idx=None, eval_synthetic=False, eval_bprna=False):
    ...
    with torch.no_grad():
        for test_set, df in test_sets.items():
            ...
            for id, sample_raw in tqdm(test_df.iterrows(), total=len(test_df)):
                ...
                if viz_idx is not None and id in viz_idx:
                    

                    sequence_str, dot_bracket_str_pred = viz(sequence, pred_mat)
                    _, dot_bracket_str_true = viz(sequence, true_mat)
                    with open("rna_structure_pred.dbn", "w") as f:
                        f.write(f">RNA Sequence\n{sequence_str}\n{dot_bracket_str_pred}\n")

                    with open("rna_structure_true.dbn", "w") as f:
                        f.write(f">RNA Sequence\n{sequence_str}\n{dot_bracket_str_true}\n")

                    try:

                        cg_pred = forgi.load_rna("rna_structure_pred.dbn", allow_many=False)
                        cg_true = forgi.load_rna("rna_structure_true.dbn", allow_many=False)

                        fvm.plot_rna(cg_pred, text_kwargs={"fontweight": "black"}, lighten=0.7,
                            backbone_kwargs={"linewidth": 3})
                        plt.savefig(f"viz/rna_structure_pred_{id}.png")
                        plt.close()

                        fvm.plot_rna(cg_true, text_kwargs={"fontweight": "black"}, lighten=0.7,
                            backbone_kwargs={"linewidth": 3})
                        plt.savefig(f"viz/rna_structure_true_{id}.png")
                        plt.close()

                    except:
                        print("Could not visualize RNA structure")
...
def viz(seq, mat):
    ...
    seq_itos = dict((y, x) for x, y in seq_stoi.items())
    # Convert the tensor to a list of integers
    sequence_list = seq.cpu().tolist()[0]  # Move to CPU and convert to list

    # Map the integers to their corresponding nucleotide characters
    sequence_str = ''.join([seq_itos[i] for i in sequence_list])

    mat = mat.cpu().numpy()

    # Initialize dot-bracket notation with all unpaired bases ('.')
    dot_bracket = ['.'] * len(sequence_list)

    # Find base pairs from the adjacency matrix
    ...
    return sequence_str, dot_bracket_str


def adjacency_matrix_to_dot_bracket_notation(adjacency_matrix):
    """Converts an adjacency matrix to dot-bracket notation RNA.

    Args:
        adjacency_matrix: A square matrix representing the adjacency matrix of the RNA
            secondary structure.

    Returns:
        A string representing the dot-bracket notation of the RNA secondary
        structure.
    """

    # Create a list to store the dot-bracket notation.
    ...
    for i in range(len(adjacency_matrix)):
        ...
        for j in range(len(adjacency_matrix[0])):
            # If the current element of the adjacency matrix is 1, then the two
            # corresponding bases are paired.
            if adjacency_matrix[i][j] == 1:
                dot_bracket_notation.append('(')
            # Otherwise, the two corresponding bases are unpaired.
            else:
                dot_bracket_notation.append('.')

    # Add a closing parenthesis for each opening parenthesis.
    for i in range(len(dot_bracket_notation)):
        if dot_bracket_notation[i] == '(':
            dot_bracket_notation.append(')')
...
if __name__ == '__main__':
    ...
    model_state_dict = state_dict['state_dict'] #["module"] #['state_dict']#
    ...
    # test_sets = pd.read_pickle("datasets/bprna_data.plk")

    viz_idx = [i*10 for i in range(100)]

    test_results, all_samples = evaluate_RNAformer(model, test_sets, viz_idx = viz_idx, eval_synthetic=args.eval_synthetic,
